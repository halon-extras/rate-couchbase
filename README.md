# Rate limiting client (Couchbase)

A rate limiting client using Couchbase.
...
```
apt-get install halon-extras-rate-couchbase
```

### RHEL

```
yum install halon-extras-rate-couchbase
```

## Prerequisites

This HSL module requires that the [couchbase](https://github.com/halon-extras/couchbase) plugin is installed.

## Exported functions

These functions needs to be [imported](https://docs.halon.io/hsl/structures.html#import) from the `extras://rate-couchbase` module path.
...
```
import { rate_fixed_window as rate } from "extras://rate-couchbase";
...
Works the same as the `rate_fixed_window` function but uses a sliding window instead of a fixed window.
